...
- **Symptom 2: Merges**  
If two developers from different teams check out a particular class and change it, merges may appear.
...
Solution is to split the functions into classes PayCalculator (calculatePay), HourReporter(reportHours), EmployeeSaver (saveEmployee). So cany change is isolated. Changes are made and the team is informed about the updates and all stakeholders are clear on what was changed.
